...
--      quakeconsole[s] = quake:new({ terminal = config.terminal,
--                                    height = 0.3,
--                                    screen = s })
--   end
...
function QuakeConsole:display()
  -- don't display console if not in the correct screen
  ...
  local client = nil
  local i = 0
  for k, c in pairs(capi.client.get(self.screen)) do
  ...
  if not client then
    ...
    awful.util.spawn(self.terminal .. " "
      .. string.format(self.argname, self.name) .. " "
      .. self.args, false, self.screen)
    return
  end
...
function QuakeConsole:new(config)
  ...
  --   config.terminal .. " " .. string.format(config.argname, config.name) .. " " ... config.args
  ...
  config.args     = config.args     or "" -- default is empty
  ...
  config.name = config.name .. '@' .. config.screen
